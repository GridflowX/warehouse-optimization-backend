...
# Pallet counts array and corresponding output file names
output_files = [f"{count}_exp1.csv" for count in pallet_counts]
...
def pick(pallet_id, arrival_time, src, dest, path):
    ...
    conn.commit()
    cursor.execute("UPDATE pallet SET departure_time = ?, flag = 1, goodspod_departure = ?, travel_time = ?, delivery_delay = ? WHERE pid = ?",
                   (final_time, goodspod_departure, travel_time, delivery_delay, pallet_id))
    conn.commit()
...
for pallet_count, output_file in zip(pallet_counts, output_files):
    ...
    with open(output_file, 'w', newline='') as csvfile:
        ...
        csvwriter.writerow([
            ...
            'departure_time', 'flag', 'goodspod_departure', 'from_stop', 'path',
            'travel_time', 'delivery_delay', 'destination_time', 'distance'
        ])
        for row in rows:
            pid, arr_stop, dest_stop, arr_time, dep_time, flag, goodspod_dep, from_stop, path_str, travel_time, delivery_delay = row
